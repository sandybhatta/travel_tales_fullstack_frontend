...
const CreatePost = ({ createModal, setCreateModal }) => {
  ...
  return (
    <div className="w-[70%] lg:h-[70px] flex justify-center overflow-x-hidden mt-5  ">
      <div
        className="w-full h-full bg-[#fff] rounded-xl shadow-xl     flex items-center justify-center  gap-4 transition-all duration-300   cursor-pointer"
        onClick={(e) => {
          e.stopPropagation();
          setCreateModal(true);
        }}
      >
        <div className=" w-full h-full  flex justify-between items-center  px-4 py-2">
          ...
          <div
            className="w-[90%] h-full  text-lg  rounded-xl  text-[#000]/50 flex items-center border px-2 shadow-2xl  "
            ref={textRef}
          ></div>
        </div>
      </div>
      ...
      {createModal && (
        <div className="w-full h-[calc(100vh-80px)] bg-[#000]/80 absolute left-[50%] -translate-x-[50%]">
          <div className="w-[70%] h-1/2 bg-white absolute left-[50%] top-[50%] -translate-x-[50%] -translate-y-[50%] rounded-2xl flex flex-col items-center justify-center gap-5">
            <i
              className="bx  bx-x text-3xl absolute cursor-pointer right-10 top-5"
              onClick={() => {
                setCreationTab("");
            ...
            <div
              className=" leckerli w-1/2 h-16 rounded-full text-center leading-[4rem] bg-red-400 text-3xl text-white cursor-pointer"
              onClick={() => {
                setCreateModal(false);
                setCreationTab("Post");
              }}
