  ...
  if (loading) {
    return (
      <div className="w-full min-h-screen bg-gray-900 text-white font-sans p-8">
        <h2 className="text-3xl font-bold mb-8 text-center md:text-left">Tagged Posts</h2>
  ...
  return (
    <div className="w-full min-h-screen bg-gray-900 text-white font-sans p-8">
      <h2 className="text-3xl font-bold mb-8 text-center md:text-left flex items-center gap-3">
        <i className='bx bx-tag-alt text-red-500'></i>
        Tagged Posts
      </h2>
      
      {posts.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-[60vh] text-gray-500">
          <i className='bx bx-tag-alt text-6xl mb-4'></i>
          <p className="text-xl">You haven't been tagged in any posts yet.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {posts.map((post) => (
